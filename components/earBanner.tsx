...
export function EarlobeHeroBanner() {
  // Filenames optimized for Earlobe & Lobuloplasty department
  // Ensure these images (or similar) exist in your public folder
  ...
  return (
    <section className="relative w-full min-h-[100dvh] flex flex-col items-center justify-center overflow-hidden bg-white pt-32 pb-4 md:pt-40 md:pb-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 opacity-30 grayscale p-4">
          {backgroundImages.map((src, i) => (
            ...
            >
              <img 
                src={src} 
                alt="Earlobe Precision Treatment Detail" 
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 flex flex-col items-center text-center">
        ...
        >
          EARLOBE <br className="sm:hidden" />
          <span className="italic font-serif text-zinc-900 font-light lowercase">& Lobuloplasty</span>
        </motion.h1>

        {/* Updated Sub-description based on your data */}
        <motion.p 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.3 }}
          className="text-zinc-600 text-sm md:text-lg lg:text-xl font-light max-w-3xl leading-relaxed mb-10 md:mb-12 uppercase tracking-[0.15em] px-4"
        >
          Advanced rejuvenation using <span className="font-medium text-zinc-900">Hyaluronic Acid</span> <br className="hidden md:block" />
          or <span className="font-medium text-zinc-900">Precision Lobuloplasty</span> under local anaesthetic.
        </motion.p>
